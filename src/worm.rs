...
pub fn draw_worm(
    ...
    worm: &Worm,
    percentage_move: f32 // from 0 to 1
) {
    ...
    let circle_tail = worm.prev_pos.lerp(
        worm.pos,
        0f32.max(percentage_move - 0.5) * 2.0 // starts after 50% and goes to 100%
    );

    let circle_head = worm.prev_pos.lerp(
        worm.pos,
        1f32.min(percentage_move * 2.0) // 2x faster to reach 100%
    );
